...
from pyteal import Bytes, Int, Expr, Addr, Seq, App
...
class AVMState:
    ...
    def globalPut(self) -> Expr:
        return Seq(
            *(
                App.globalPut(Bytes(self.field_to_key(f)), self.encode_to_avm(f))
                for f in dataclasses.fields(self)
            )
        )

    def localPut(self, account: Expr) -> Expr:
        return Seq(
            *(
                App.localPut(
                    account, Bytes(self.field_to_key(f)), self.encode_to_avm(f)
                )
                for f in dataclasses.fields(self)
            )
        )
